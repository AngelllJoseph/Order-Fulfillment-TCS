...
import PartsStock from './production/PartsStock';
import HubCapacity from './production/HubCapacity';
import AISuggestions from './production/AISuggestions';
import Notifications from './production/Notifications';
import ActivityLog from './production/ActivityLog';
import Profile from './production/Profile';
...
const ProductionDashboard = () => {
    ...
    const navItems = [
        { id: 'production-overview', label: 'Production Overview', icon: Factory },
        { id: 'assigned-orders', label: 'Active Orders', icon: ClipboardList },
        { id: 'production-stages', label: 'Production Stages', icon: Settings },
        { id: 'inventory-management', label: 'Inventory Management', icon: Package },
        { id: 'parts-stock', label: 'Parts & Stock', icon: Puzzle },
        { id: 'hub-capacity', label: 'Hub Capacity', icon: BarChart2 },
                ...
                <header style={styles.topbar}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        <div style={styles.searchBox}>
                            ...
                            <input
                                type="text"
                                placeholder="Search orders, inventory, hubs..."
                    ...
                    <div style={styles.topActions}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginLeft: '0.5rem' }}>
                            ...
                            <div style={{ display: 'none', md: 'block' }}>
                                <div style={{ fontSize: '0.875rem', fontWeight: 600 }}>{user?.first_name || 'Production Lead'}</div>
                                <div style={{ fontSize: '0.75rem', color: colors.textMuted }}>Manufacturing Lead</div>
                            </div>
                        </div>
                    </div>
                </header>
                ...
                <main style={styles.content}>
                    <div style={styles.header}>
                        ...
                        <p style={{ color: colors.textMuted, marginTop: '0.5rem' }}>
                            Production Workflows • {user?.role || 'Manufacturing Lead'}
                        </p>
                    </div>
                    ...
                    <div style={{ marginTop: '1rem' }}>
                        ...
                        {activeTab === 'parts-stock' && <PartsStock colors={colors} darkMode={darkMode} />}
                        {activeTab === 'hub-capacity' && <HubCapacity colors={colors} darkMode={darkMode} />}
                        {activeTab === 'ai-suggestions' && <AISuggestions colors={colors} darkMode={darkMode} />}
                        {activeTab === 'notifications' && <Notifications colors={colors} darkMode={darkMode} />}
                        {activeTab === 'activity-log' && <ActivityLog colors={colors} darkMode={darkMode} />}
                        {activeTab === 'profile' && <Profile colors={colors} darkMode={darkMode} />}
                    </div>
                </main>
            </div>
